...
def gen_LrgSector(arr, yStart, xStart):
    ...
    lrgSector = [[None for _ in range(GRID_COLS)] for _ in range(GRID_ROWS)]

    def roomBuilder(arr, dArr):


        h = random.randrange(6, 12)
        w = random.randrange(6, 12)
        longestDim = h
        if w > h:
            longestDim = w
        ...
        while i < len(arr) -1:
            for j in range(i, len(arr)):
                ...
                if j != i:
                    ...
                    if abs(arr[i][0] - arr[j][0]) > 10 and abs(arr[i][1] - arr[j][1]) > 10 and abs(arr[i][0] - arr[j][0]) < 30 and abs(arr[i][1] - arr[j][1]) < 30:
                        path = []
